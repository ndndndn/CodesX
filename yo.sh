...
function Redirect()
{
 window.location="http://facebook.com/radzvpn";
}
...
sed -i 's|export KEY_EMAIL="me@myhost.mydomain"|export KEY_FB="WWW.FACEBOOK.COM/RADZVPN"|' /etc/openvpn/easy-rsa/vars
...
sed -i 's|export KEY_NAME="EasyRSA"|export KEY_NAME="RADZ"|' /etc/openvpn/easy-rsa/vars
...
# Modded by RadzVPN
# TEAM RadzVPN | Epiphany | www.radzvpn.ml
# FB Page : https://facebook.com/radzvpn
...
echo ""  | tee -a log-install.txt
echo "* Join TEAM Epiphany Discord Group Chat"  | tee -a log-install.txt
echo "   - Link		: https://discord.gg/EHq4XjH"  | tee -a log-install.txt
echo ""  | tee -a log-install.txt
